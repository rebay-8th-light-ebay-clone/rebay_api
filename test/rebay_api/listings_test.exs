defmodule RebayApi.ListingsTest do
  ...
  describe "items" do
    ...
    @update_attrs %{category: "some updated category", description: "some updated description", image: "http://www.some-updated-image.foo", title: "some updated title"}
    ...
    test "update_item/2 with end date returns error changeset" do
      item = item_fixture()
      invalid_attrs = Map.put(@valid_attrs, :end_date, "2031-05-07T14:00:00Z")
      assert {:error, %Ecto.Changeset{}} = Listings.update_item(item, invalid_attrs)
      assert item == Listings.get_item!(item.uuid)
    end
